...
from constants.tokens import TokenType, HeaderToken, CookieToken
...
class WebRefreshTokenAuthentication(authentication.BaseAuthentication):
    '''Authenticate user through Jwt Refresh Token.'''

    def authenticate(self, request):
        try:
            # getting validation success and payload from authentication token
            bearer_token = request.COOKIES.get(CookieToken.REFRESH_TOKEN)
            success, payload = Jwt.validate(bearer_token)
            payload_data = payload['data']
